...
ESTADISTICAS_URL = f"{ENCUESTA_URL}/actividad-fisica/resultados/estadisticas"
...
def test_estadisticas_sin_encuestas(client, act_fisica_headers):
    res = client.get(ESTADISTICAS_URL, headers=act_fisica_headers)
...
def test_estadisticas_con_una_encuesta(client, auth_headers, act_fisica_headers):
...
def test_estadisticas_ordena_peor_facultad_primero(client, act_fisica_headers):
    peor = {
        **ENCUESTA_PAYLOAD,
        "facultad": "Facultad Baja",
        **{k: 1 for k in ENCUESTA_PAYLOAD if k.startswith("af_item_")},
    }
    mejor = {
        **ENCUESTA_PAYLOAD,
        "facultad": "Facultad Alta",
        **{k: 4 for k in ENCUESTA_PAYLOAD if k.startswith("af_item_")},
    }

    for payload, correo in [(peor, "baja_af@vitalis.com"), (mejor, "alta_af@vitalis.com")]:
