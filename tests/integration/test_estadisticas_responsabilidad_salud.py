...
ESTADISTICAS_URL = f"{ENCUESTA_URL}/responsabilidad-salud/resultados/estadisticas"
...
def test_estadisticas_sin_encuestas(client, resp_salud_headers):
    res = client.get(ESTADISTICAS_URL, headers=resp_salud_headers)
...
def test_estadisticas_con_una_encuesta(client, auth_headers, resp_salud_headers):
...
def test_estadisticas_ordena_peor_facultad_primero(client, resp_salud_headers):
    peor = {
        **ENCUESTA_PAYLOAD,
        "facultad": "Facultad Baja",
        **{k: 1 for k in ENCUESTA_PAYLOAD if k.startswith("rs_item_")},
    }
    mejor = {
        **ENCUESTA_PAYLOAD,
        "facultad": "Facultad Alta",
        **{k: 4 for k in ENCUESTA_PAYLOAD if k.startswith("rs_item_")},
    }

    for payload, correo in [(peor, "baja_rs@vitalis.com"), (mejor, "alta_rs@vitalis.com")]:
