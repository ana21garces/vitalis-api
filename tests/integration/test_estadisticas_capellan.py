...
ESTADISTICAS_URL = f"{ENCUESTA_URL}/capellan/psicologia-positiva/estadisticas"
...
def test_estadisticas_sin_encuestas(client, capellan_headers):
    res = client.get(ESTADISTICAS_URL, headers=capellan_headers)
    assert res.status_code == 200
    data = res.json()
    assert data["poblacion_general"]["total"] == 0
    assert data["poblacion_general"]["promedio_indice"] == 0.0
    assert data["por_facultad"] == []


def test_estadisticas_con_una_encuesta(client, auth_headers, capellan_headers):
    ...
    assert len(data["por_facultad"]) == 1
    facultad = data["por_facultad"][0]
    assert facultad["facultad"] == "Ingenieria"
    assert facultad["conteo"]["total"] == 1


def test_estadisticas_ordena_peor_facultad_primero(client, capellan_headers):
    """Dos facultades con distinto índice: la de peor promedio debe salir primero,
    para que el capellán vea de una vez cuál necesita más atención."""
    peor = {
        **ENCUESTA_PAYLOAD,
        "facultad": "Facultad Baja",
        **{k: 1 for k in ENCUESTA_PAYLOAD if k.startswith("pp_item_")},
    }
    mejor = {
        **ENCUESTA_PAYLOAD,
        "facultad": "Facultad Alta",
        **{k: 4 for k in ENCUESTA_PAYLOAD if k.startswith("pp_item_")},
    }

    for payload, correo in [(peor, "baja@vitalis.com"), (mejor, "alta@vitalis.com")]:
