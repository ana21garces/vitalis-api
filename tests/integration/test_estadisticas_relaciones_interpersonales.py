...
ESTADISTICAS_URL = f"{ENCUESTA_URL}/relaciones-interpersonales/resultados/estadisticas"
...
def test_estadisticas_sin_encuestas(client, ri_headers):
    res = client.get(ESTADISTICAS_URL, headers=ri_headers)
...
def test_estadisticas_con_una_encuesta(client, auth_headers, ri_headers):
...
def test_estadisticas_ordena_peor_facultad_primero(client, ri_headers):
    peor = {
        **ENCUESTA_PAYLOAD,
        "facultad": "Facultad Baja",
        **{k: 1 for k in ENCUESTA_PAYLOAD if k.startswith("ri_item_")},
    }
    mejor = {
        **ENCUESTA_PAYLOAD,
        "facultad": "Facultad Alta",
        **{k: 4 for k in ENCUESTA_PAYLOAD if k.startswith("ri_item_")},
    }

    for payload, correo in [(peor, "baja_ri@vitalis.com"), (mejor, "alta_ri@vitalis.com")]:
