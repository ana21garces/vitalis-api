...
ESTADISTICAS_URL = f"{ENCUESTA_URL}/nutricion/resultados/estadisticas"
...
def test_estadisticas_sin_encuestas(client, n_headers):
    res = client.get(ESTADISTICAS_URL, headers=n_headers)
...
def test_estadisticas_con_una_encuesta(client, auth_headers, n_headers):
...
def test_estadisticas_ordena_peor_facultad_primero(client, n_headers):
    peor = {
        **ENCUESTA_PAYLOAD,
        "facultad": "Facultad Baja",
        **{k: 1 for k in ENCUESTA_PAYLOAD if k.startswith("n_item_")},
    }
    mejor = {
        **ENCUESTA_PAYLOAD,
        "facultad": "Facultad Alta",
        **{k: 4 for k in ENCUESTA_PAYLOAD if k.startswith("n_item_")},
    }

    for payload, correo in [(peor, "baja_n@vitalis.com"), (mejor, "alta_n@vitalis.com")]:
